...
public class Black implements Piece{
    ...
    public Black(Colour colour, Position position, Type type) {
        this.colour = colour;
     ...
     */
    private void getOptions(Position start, Position end, LinkedList<Position> visited,
                            LinkedList<Position> route, Map<Integer, List<Position>> lists){
        route.add(start);
        visited.add(start);
    ...
    /**
     ...
     - the destination position is free and between destination and current positions exists a 'RED' piece.
     */
    private List<Position> getNeighbours(Position current){
        ...
        if (isValid(current)){
            ...
            if (isValid(temp) && isValid(freeSpot) && board[temp.getX()][temp.getY()] instanceof Red && board[freeSpot.getX()][freeSpot.getY()] == null){
                list.add(freeSpot);
            }
    ...
    public String toString() {
        if (this.type == Type.MAN){
            return "\033[0;30mx\033[0m";
        } else {
            return "\033[1;90m\033[1;30mX\033[0m";
        }
    }
